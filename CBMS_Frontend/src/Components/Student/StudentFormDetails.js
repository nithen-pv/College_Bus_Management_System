...
const StudentFormDetails = (props) => {
  ...
  const yearList = [];
  for (let index = 2018; index < 2036; index++) {
    yearList.push(index);
  }
  ...
  const createStudent = async (e) => {
    ...
    const res = await Services.createStudent(studentData);
    console.log(res);
    const userInfo = {
      username: name + rollNo + "@mes",
    ...
    await AuthService.register(userInfo).then((data) => {
      console.log("Student Login Created", data);
    });
    ...
    <form onSubmit={createStudent}>
      <Card>
        ...
        >
          ...
          studentDetail.year &&
          studentDetail.bus &&
          studentDetail.busStop &&
          studentDetail.busFee &&
          studentDetail.paymentStatus !== "" ? (
            <Button
              color="primary"
              variant="contained"
              type="submit"
